...
import { Search, Star, HardDrive } from 'lucide-react'
    ...
    <section className="mx-auto max-w-6xl px-5 pb-24 pt-28 sm:px-8">
      <div className="mb-10">
        ...
        <p className="mt-3 max-w-xl text-muted">
          Official entries ship with the site. Local uploads stay in your browser until you open a
          PR.
        </p>
      </div>
      ...
      <div className="mb-8 flex flex-col gap-3 sm:flex-row sm:items-center">
        <div className="relative flex-1">
          <Search className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-dim" size={16} />
          <input
            value={q}
        ...
        {filtered.map((d) => (
          ...
          >
            <div className="mb-3 flex items-start justify-between gap-2">
              ...
              {d.featured && <Star size={14} className="shrink-0 text-accent" />}
            </div>
      ...
      {!loading && filtered.length === 0 && (
        <p className="mt-10 text-center text-muted">
          No dumps match. Try clearing filters or{' '}
